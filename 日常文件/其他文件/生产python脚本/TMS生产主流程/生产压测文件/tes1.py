...
class Test(TaskSet):
    ...
    def create_order(self):#下单
        ...
        with self.client.get('/tms-saas-web/pdd/shipment/status?tracking_number=123'
                             '', headers = header, name = "测试", catch_response = True) as response:
...
class websitUser(HttpUser):
    tasks = [Test]
    host = "https://cb-tms-kec-de.kec-app.com"
    min_wait = 1000  # 单位为毫秒
    max_wait = 2000  # 单位为毫秒