...
from TMS生产主流程.OPS.Check_Weight import Check_Weight
from TMS生产主流程.OPS.Close_Box import Close_Box
from TMS生产主流程.OPS.Inbound import OPS_Inbound
from TMS生产主流程.OPS.Outbound import Outbound_Scan
from TMS生产主流程.OPS.Package_Scan import Package_Scan
from TMS生产主流程.TMS.mawb import create, scan_box, close_mawb
from TMS生产主流程.TMS.shipment import shipment_add, shipment_scan, shipment_close
from TMS生产主流程.TMS.status import status
from TMS生产主流程.order.Big_Bag_Create import Big_Bag_Create
from TMS生产主流程.order.Order_Create import Order_Create
from TMS生产主流程.properties.GetProperties import getProperties
from TMS生产主流程.public.Ops_Login import Ops_Login
from TMS生产主流程.public.Pos_Login import Pos_Login
from TMS生产主流程.public.tms_Login import tms_login
...
class MyTestCase(unittest.TestCase):
    ...
    def test_case_order(self):
        ...
        if x>=1:
            for i in range(1):
                ...
                if "失败" in tracking_number:
                    fail=fail+1
                # tracking_number = "2010555613060039"
        ...
        # time.sleep(1)
        # status(tracking_number,"EN",tms_token,properties,"出口清关销毁")
        time.sleep(1)
        status(tracking_number,"FX",tms_token,properties,"出口清关完成")
        time.sleep(1)
        status(tracking_number,"OC",tms_token,properties,"干线出发")#CAINIAO_GLOBAL_LINEHAUL_DEPARTURE_CALLBACK
        time.sleep(1)
        status(tracking_number,"OF",tms_token,properties,"干线到达")
        time.sleep(1)
        ...
        time.sleep(100)
        status(tracking_number,"HL",tms_token,properties,"干配交接")
        time.sleep(1)
        status(tracking_number,"ZY",tms_token,properties,"到达站点")
        time.sleep(1)
