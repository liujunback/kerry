...
def select_centre_id_OR_client_ids(properties,login_data,order_number):#查询客户和仓库
    """优化版订单查询函数"""
    # 基础URL配置
    ...
    params = {
        ...
        "columns[9][search][value]": "2025-08-28 00:00:00 - " + (datetime.datetime.now()).strftime(
            '%Y-%m-%d ') + "23:59:59",
    ...
    headers = {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Cookie': 'XSRF-TOKEN=' + login_data['cookies']['XSRF-TOKEN'] + '; laravel_session=' +
                  login_data['cookies']['laravel_session']
    }
